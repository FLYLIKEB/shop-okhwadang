...
export class SeedCustomerServicePages1776400000000 implements MigrationInterface {
  name = 'SeedCustomerServicePages1776400000000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      INSERT IGNORE INTO \`pages\` (\`slug\`, \`title\`, \`template\`, \`is_published\`) VALUES
      ('support', '고객센터', 'default', 1)
    `);

    const [supportPage] = await queryRunner.query(`SELECT id FROM pages WHERE slug = 'support'`);
    if (supportPage) {
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, 'split_content', ?, 0, 1)`,
        [
          supportPage.id,
          JSON.stringify({
    ...
    await queryRunner.query(`
      INSERT IGNORE INTO \`pages\` (\`slug\`, \`title\`, \`template\`, \`is_published\`) VALUES
      ('shipping', '배송 안내', 'default', 1)
    `);

    const [shippingPage] = await queryRunner.query(`SELECT id FROM pages WHERE slug = 'shipping'`);
    if (shippingPage) {
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, 'text_content', ?, 0, 1)`,
        [
          shippingPage.id,
          JSON.stringify({
    ...
    await queryRunner.query(`
      INSERT IGNORE INTO \`pages\` (\`slug\`, \`title\`, \`template\`, \`is_published\`) VALUES
      ('returns', '반품 및 교환', 'default', 1)
    `);

    const [returnsPage] = await queryRunner.query(`SELECT id FROM pages WHERE slug = 'returns'`);
    if (returnsPage) {
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, 'text_content', ?, 0, 1)`,
        [
    ...
    await queryRunner.query(`
      INSERT IGNORE INTO \`pages\` (\`slug\`, \`title\`, \`template\`, \`is_published\`) VALUES
      ('terms', '이용약관', 'default', 1)
    `);

    const [termsPage] = await queryRunner.query(`SELECT id FROM pages WHERE slug = 'terms'`);
    if (termsPage) {
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, 'text_content', ?, 0, 1)`,
        [
    ...
    await queryRunner.query(`
      INSERT IGNORE INTO \`pages\` (\`slug\`, \`title\`, \`template\`, \`is_published\`) VALUES
      ('privacy', '개인정보처리방침', 'default', 1)
    `);

    const [privacyPage] = await queryRunner.query(`SELECT id FROM pages WHERE slug = 'privacy'`);
    if (privacyPage) {
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, 'text_content', ?, 0, 1)`,
        [
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    const slugs = ['support', 'shipping', 'returns', 'terms', 'privacy'];
    for (const slug of slugs) {
      const [page] = await queryRunner.query(`SELECT id FROM pages WHERE slug = ?`, [slug]);
      if (page) {
