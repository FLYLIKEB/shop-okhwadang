...
export class SeedCustomerServicePages1776400000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const pages = await queryRunner.query(`SELECT id, slug FROM \`pages\` WHERE slug IN ('support','shipping','returns','terms','privacy')`);
    for (const page of pages) {
      const [existing] = await queryRunner.query(`SELECT id FROM \`page_blocks\` WHERE page_id = ?`, [page.id]);
      ...
      const blockType = page.slug === 'support' ? 'split_content' : 'text_content';
      await queryRunner.query(
        `INSERT IGNORE INTO \`page_blocks\` (\`page_id\`, \`type\`, \`content\`, \`sort_order\`, \`is_visible\`) VALUES (?, ?, ?, 0, 1)`,
        [page.id, blockType, JSON.stringify(content)],
      );
    }
  }
