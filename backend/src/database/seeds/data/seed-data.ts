...
export const pages: SeedPage[] = [
  ...
  { slug: 'archive', title: '아카이브', titleEn: 'Archive', template: 'default', isPublished: true },
  { slug: 'collection', title: '콜렉션', titleEn: 'Collection', template: 'default', isPublished: true },
...
export const pageBlocks: SeedPageBlock[] = JSON.parse(JSON.stringify([
  ...
  // ── 아카이브 페이지 ──
  {
    ...
    content: {
      ...
      description: '자사호의 생명은 흙에서 시작됩니다. 산지별 니료의 특성, 채토부터 소성까지의 공정, 그리고 흙과 함께 살아온 장인들의 이야기를 기록합니다.',
      description_en: 'The life of a Yixing teapot begins with the clay. We record the character of clays by origin, the stages from quarrying to firing, and the stories of masters who have lived with the earth.',
      bgColor: 'foreground',
    },
    sortOrder: -1,
    isVisible: true,
  },
  {
    ...
    content: {
      ...
      layout: 'alternating',
      items: niloTypes.map((item) => ({
        id: `nilo-${item.sortOrder}`,
        color: item.color,
        nameEn: item.name,
        nameKo: item.nameKo,
        region: item.region,
        description: item.description,
        characteristics: JSON.parse(item.characteristics) as string[],
  ...
  // ── 콜렉션 페이지 ──
  {
    ...
    content: {
      ...
      description: '자사호의 개성은 흙과 형태에서 비롯됩니다. 니료의 색과 질감, 형태의 선과 비례로 나만의 자사호를 찾아보세요.',
      description_en: 'The character of a Yixing teapot comes from clay and form. Discover yours through color, texture, line, and proportion.',
      bgColor: 'foreground',
    },
    sortOrder: -1,
    isVisible: true,
  },
  {
    ...
    content: {
      ...
      layout: 'grid-3',
      items: collections.filter((item) => item.type === 'clay').map((item) => ({
        id: `collection-clay-${item.sortOrder}`,
  ...
  },
  {
    ...
    content: {
      ...
      columns: 3,
      items: collections.filter((item) => item.type === 'shape').map((item) => ({
        id: `collection-shape-${item.sortOrder}`,
        imageUrl: '',
        name: item.nameKo,
        description: item.description ?? '',
        href: item.productUrl ?? '',
        hrefLabel: `${item.nameKo} 보기`,
      })),
    },
    sortOrder: 1,
    isVisible: true,
