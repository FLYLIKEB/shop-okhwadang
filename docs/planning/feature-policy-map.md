...
- 글로벌 정책: 언어는 한국어/영어만 지원한다. 기존 일본어/중국어 DB 컬럼은 마이그레이션 호환용 deprecated 필드이며, 신규 UI/API/시드/DTO에서는 ja/zh 값을 입력·노출·갱신하지 않는다.
...
- 스마트스토어 리뷰는 `external_reviews`로 별도 저장하고 상품 상세 리뷰 목록에 병합 표시한다.
...
- 스마트스토어 리뷰 데이터는 관리자 수동 CSV/JSON 가져오기를 우선 사용한다. 크롤링은 금지하고 공식 리뷰 API 권한이 확인되면 별도 자동 동기화로 확장한다.
