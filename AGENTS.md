...
This repository previously used Claude Code + OMC. In Codex/OMX sessions, this file is the top-level contract. Legacy Claude files remain the project reference source, but `~/.claude/CLAUDE.md` is not auto-loaded here.
...
- Make the most reasonable decision autonomously when the next step is clear and reversible.
- Complete the task fully, then report results.
- Prefer the existing project structure over new abstractions.
- Stop and report on direct conflicts instead of forcing through them.
- For app-affecting changes, restart local services with `bash scripts/start-local.sh` before finishing unless the user explicitly says not to.
...
Critical project rules pulled forward from the Claude setup:
- Frontend design must follow `DESIGN.md`.
- API calls in frontend feature code go through `src/lib/api.ts`; do not add raw `fetch()` in feature code.
- Use `handleApiError()` for frontend API error extraction and `formatCurrency()` for price formatting.
- Hardcoded user-facing Korean literals in frontend components are forbidden; use `next-intl` keys and update all locale files.
- Backend public endpoints must explicitly use `@Public()`.
- NestJS backend uses Logger and built-in exceptions only; no committed `console.log`.
- Entity/schema changes require the paired migration and `cd backend && npm run test:e2e`.
- Never expose `.env`, `.pem`, `.key`, or credentials file contents unless the user explicitly requests it and the task genuinely requires it.

Codex/OMX skill locations for this repo:
- Project OMX skills: `./.codex/skills/*`
- Project OMX prompts: `./.codex/prompts/*`
- Legacy Claude skills for reference/migration only: `~/.claude/skills/*`

Project custom skills available in this repo:
- `start-server`
- `db-seed`
- `perf-check`
- `env-sync`
